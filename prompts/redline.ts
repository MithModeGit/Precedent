...
Return one redline object per issue you choose to address (a clause may yield more than one, and a missing clause is added as a new redline whose originalText is the closest existing clause text). Each redline must include the originalText exactly as provided (or the closest clause text when adding a missing clause or flagging a document-wide issue), the proposedText, the priority tier, a rationale, a citation, and a counterparty prediction.`
}
